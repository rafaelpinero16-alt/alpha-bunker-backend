const app = {
    ...
    updateProfileUI() {
        ...
        if (kycStatusEl) {
            if (kycStatus === 'verified' || isAdminUser) {
                ...
                kycStatusEl.innerText = `NO VERIFICADO ⚠️${warningText}`; kycStatusEl.className = `text-xs font-black uppercase text-neutral-400`;
        ...
        if (creatorTools) {
            if (userRole === 'creator' || isAdminUser) {
                creatorTools.classList.remove('hidden'); if (creatorSubBox) creatorSubBox.classList.remove('hidden');
            } else {
                creatorTools.classList.add('hidden'); if (creatorSubBox) creatorSubBox.classList.add('hidden');
            }
        }
    },
    ...
    openUploadPanel() {
        this.initUserId();
        const kycStatus = localStorage.getItem('alpha_kyc_status') || 'unverified', userRole = this.userData?.role || 'fan', isAdminUser = this.isAdminUser();
