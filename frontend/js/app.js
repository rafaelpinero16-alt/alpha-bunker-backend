const app = {
    ...
    // ⚡ WEBSOCKETS PARA CHAT CRM Y CHAT GLOBAL INDEPENDIENTE
    chatSocket: null,
    ...
    // ✨ EFECTO VISUAL DE FUEGOS ARTIFICIALES (CÍAN Y FUCSIA) PARA PAGOS EXITOSOS
    triggerFireworks() {
                ...
                if (window.Telegram?.WebApp?.openInvoice) {
                    window.Telegram.WebApp.openInvoice(data.invoice_link, async (status) => {
                        if (status === 'paid') {
                            ...
                            this.triggerFireworks(); // 🎆 Efecto visual de fuegos artificiales
    ...
    // 📹 Validación de Rango/Suscripción para la Videollamada (Independiente)
    startVideoCall() { 
        this.haptic('medium');
        this.initUserId();
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    updateProfileUI() {
        ...
        // 🛡️ REGLA: Los fans NO tienen Tip Menu ni ven herramientas de Creador. Esta lógica ya lo blinda.
        const creatorTools = document.getElementById('prof-creator-tools');
        const creatorSubBox = document.getElementById('prof-creator-subscription-box');
        const userRole = localStorage.getItem('alpha_user_role') || this.userData?.role;
        if (creatorTools) {
    ...
    // 🛡️ ACTUALIZADO: Permite que Fans también abran el panel para publicar
    openUploadPanel() {
        this.initUserId();
        const kycStatus = localStorage.getItem('alpha_kyc_status') || 'unverified';
        const userRole = localStorage.getItem('alpha_user_role') || this.userData?.role;
